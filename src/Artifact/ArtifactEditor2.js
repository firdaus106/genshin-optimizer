...
import UploadDisplay from "./UploadDisplay2";
...
export default class ArtifactEditor extends React.Component {
  ...
  saveArtifact = (id) => {
    ...
    if (!artToSave.maximumEfficiency)
  ...
  setSetKey = (setKey) =>
    this.setState((state) => {
      ...
      let level =
        !state.level || state.level > numStars * 4 ? numStars * 4 : state.level;
  ...
  setMainStatKey = (mainStatKey) =>
    this.setState((state) => {
      state.substats.forEach(
        (substat, index) =>
          substat.key &&
          substat.key === mainStatKey &&
          (state.substats[index] = { key: "", value: 0 })
      );
      return { mainStatKey };
    });
  ...
  setSlotKey = (slotKey) =>
    this.setState((state) => {
      ...
      for (const mainStatKey of mainstats)
        if (
          !state.substats.some((substat) =>
            substat && substat.key ? substat.key === mainStatKey : false
          )
        )
  ...
  checkDuplicate = () => {
    let {
      ...
      slotKey = "",
      mainStatKey = "",
      substats = deepClone(ArtifactEditor.initialState.substats),
    } = this.state;
    let dupId = null;
    let dup = false;
    if (!id && setKey && slotKey && numStars && mainStatKey) {
      ...
      let artifacts = Object.values(
        ArtifactDatabase.getArtifactDatabase()
      ).filter((art) => {
        ...
        for (const artSubstat of art.substats) {
          ...
          let substat = substats.find(
            (substat) =>
              substat.key === artSubstat.key &&
              (substat.value > artSubstat.value ||
                Artifact.subStatCloseEnough(
                  substat.key,
                  substat.value,
                  artSubstat.value
                ))
  ...
  randomizeArtifact = () => {
    ...
    //choose star
    state.numStars = getRandomElementFromArray(
      Artifact.getRarityArr(state.setKey)
    );
    //choose piece
    state.slotKey = getRandomElementFromArray(
      Object.keys(Artifact.getPieces(state.setKey))
    );
    //choose mainstat
    state.mainStatKey = getRandomElementFromArray(
      Artifact.getSlotMainStatKeys(state.slotKey)
    );

    //choose initial substats from star
    let numOfInitialSubStats = getRandomIntInclusive(
    ...
    let RollStat = (subStatKey) =>
      getRandomElementFromArray(
        Artifact.getSubstatRollData(subStatKey, state.numStars)
      );
    ...
    for (let i = 0; i < numOfInitialSubStats; i++) {
      ...
      remainingSubstats = remainingSubstats.filter(
        (key) => key !== substat.key
      );
    ...
    for (let i = 0; i < numUpgradesOrUnlocks; i++) {
      ...
      if (!Number.isInteger(substat.value))
        substat.value = parseFloat(substat.value.toFixed(1));
    }
    this.props.cancelEdit?.();
    this.setState(state);
  };
  componentDidUpdate = (prevProps, prevState) => {
    if (
      this.props.artifactIdToEdit &&
      prevProps.artifactIdToEdit !== this.props.artifactIdToEdit
    )
      this.setState(
        deepClone(ArtifactDatabase.get(this.props.artifactIdToEdit))
      );
  };
  render() {
      ...
      <Card bg="darkcontent" text="lightfont">
          ...
          <Row className="mb-n2">
            ...
            <Col xs={12} lg={6} className="mb-2">
              <InputGroup className="w-100 d-flex">
                  ...
                  <Dropdown.Menu>
                    {[5, 4, 3].map((star, i) => (
                      <React.Fragment key={star}>
                        ...
                        <Dropdown.ItemText>
                          Max Rarity <Stars stars={star} />
                        </Dropdown.ItemText>
                        {Artifact.getSetsByMaxStarEntries(star).map(
                          ([key, setobj]) => (
                            <Dropdown.Item
                              key={key}
                ...
                <DropdownButton
                  ...
                  title={numStars > 0 ? <Stars stars={numStars} /> : "Rarity"}
                  disabled={!setKey}
                >
                  {Artifact.getStars().map((star, index) => (
                    <Dropdown.Item
                      key={index}
                      disabled={!Artifact.getRarityArr(setKey).includes(star)}
                      onClick={() =>
              ...
              <InputGroup>
                ...
                <InputGroup.Append>
                  <Button
                    ...
                    disabled={!setKey || level === 0}
                  >
                    0
                  </Button>
                  <Button
                    onClick={() => this.setLevel(level - 1)}
                    disabled={!setKey || level === 0}
                  >
                    -
                  </Button>
                  <Button
                    onClick={() => this.setLevel(level + 1)}
                    disabled={!setKey || level === numStars * 4}
                  >
                    +
                  </Button>
                  <Button
                    onClick={() => this.setLevel(numStars * 4)}
                    disabled={!setKey || level === numStars * 4}
                  >
                    {numStars * 4}
                  </Button>
              ...
              <InputGroup>
                ...
                >
                  {Object.keys(Artifact.getPieces(setKey)).map((slotKey) => (
                    <Dropdown.Item
                      key={slotKey}
                      onClick={() => this.setSlotKey(slotKey)}
                    >
            ...
            <Col xs={12} lg={6} className="mb-2">
              <InputGroup>
                ...
                >
                  <Dropdown.ItemText>
                    Select a Main Artifact Stat{" "}
                  </Dropdown.ItemText>
                  {Artifact.getSlotMainStatKeys(slotKey).map((mainStatKey) => (
                    <Dropdown.Item
                      key={mainStatKey}
                      onClick={() => this.setMainStatKey(mainStatKey)}
                    >
            ...
            {substats.map((substat, index) => {
              const remainingSubstats = this.getRemainingSubstats(
                mainStatKey,
                substats
              );
              return (
                <Col key={"substat" + index} className="mb-2" xs={12} lg={6}>
                  <SubStatInput
                    {...{
                      index,
                      substat,
              ...
              <Card bg="lightcontent" text="lightfont">
                <Card.Body className="py-1 px-2">
                  <Row>
                    ...
                    <Col xs="auto">
                      <PercentBadge
                        valid={!errMsgs.length}
                        percent={currentEfficiency}
                      >
                      ...
                      <OverlayTrigger
                        ...
                        overlay={
                          <Popover>
                            ...
                            <Popover.Content>
                              <span>
                                Every 4 artifact upgrades, you get a substat
                                roll. <strong>Substat Efficiency</strong>{" "}
                                calculates how high the substat rolled as a
                                percentage.
                              </span>
                            </Popover.Content>
                          </Popover>
                      ...
                      <OverlayTrigger
                        ...
                        overlay={
                          <Popover>
                            ...
                            <Popover.Content>
                              <span>
                                The <strong>Maximum Substat Efficiency</strong>{" "}
                                of an artifact calculates the efficiency if the
                                remaining upgrades rolled their maximum values.
                              </span>
                            </Popover.Content>
                          </Popover>
                        }
            ...
            <Col xs={12} className="mb-2">
              <UploadDisplay
                ...
                reset={(reset) => (this.uploadDisplayReset = reset)}
              />
            </Col>
              ...
              <Col xs={12} className="mb-2">
                <Row className="d-flex justify-content-around mb-n2">
                  <Col lg={4} md={6} className="mb-2">
                    ...
                    <div>
                      <ArtifactCard artifactObj={this.state} />
                    </div>
                  </Col>
                  <Col lg={4} md={6} className="mb-2">
                    <h6 className="text-center">
                      {dupId
                        ? `Detected ${dup ? "Duplicate" : "Upgraded"} Artifact`
                        : `Before Edit`}
                    </h6>
                    <div>
                      <ArtifactCard artifactId={dupId || id} />
                    </div>
                  </Col>
                </Row>
              </Col>
        ...
        <Card.Footer>
          <Button
            ...
            disabled={ArtifactDatabase.isInvalid(this.state) || errMsgs.length}
            variant={dupId ? "warning" : "primary"}
          >
            {id ? "Save Artifact" : "Add Artifact"}
          </Button>
          <Button
            className="mr-2"
            onClick={this.clearArtifact}
            variant="success"
          >
            Clear
          </Button>
          {process.env.NODE_ENV === "development" && (
            <Button variant="info" onClick={this.randomizeArtifact}>
              Randomize
            </Button>
          )}
          {Boolean(dupId) && (
            <Button
              ...
              onClick={() => this.saveArtifact(dupId)}
              disabled={
                ArtifactDatabase.isInvalid(this.state) || errMsgs.length
              }
              variant="success"
            >
              Update Artifact
...
function SubStatInput({
  index,
  substat: { key, value, rolls, efficiency },
  numStars,
  remainingSubstats = [],
  setSubStat,
}) {
  ...
  if (!error && numStars) {
    ...
    let totalAllowableRolls =
      Artifact.getNumUpgradesOrUnlocks(numStars) -
      (4 - Artifact.getBaseSubRollNumHigh(numStars)) +
      1;
    if (rollNum > totalAllowableRolls)
      error = `Substat cannot be rolled more than ${totalAllowableRolls} times.`;
  }
  let rollLabel = null;
  if (!error) {
    const rollBadge = (
      <Badge
        variant={rollNum === 0 ? "secondary" : `${rollNum}roll`}
        className="text-darkcontent"
      >
    ...
    const rollArr = rolls?.map((val, i) => {
      ...
      return (
        <span key={i} className={`mr-2 text-${displayNum}roll`}>
          {val}
        </span>
      );
    });
    const rollDataDisplay = Boolean(rollData.length) && (
      <span className="float-right text-right">
        ...
        <ButtonGroup size="sm">
          {rollData.map((v, i, arr) => (
            <Button
              key={i}
              variant={`${6 - (arr.length - 1 - i)}roll`}
              className="py-0 text-darkcontent"
              onClick={() =>
                setSubStat?.(
                  index,
                  key,
                  isPercentStat ? parseFloat((value + v).toFixed(1)) : value + v
                )
              }
            >
              {v}
  ...
  return (
    <Card bg="lightcontent" text="lightfont">
      <InputGroup>
        ...
        >
          {Boolean(key) && (
            <Dropdown.Item key={key} onClick={() => setSubStat?.(index, "")}>
              No Substat
            </Dropdown.Item>
          )}
        ...
        <InputGroup.Append>
          ...
          <InputGroup.Text>
            <PercentBadge valid={!error || !key} percent={efficiency}>
              {key
                ? !error
                  ? `${(efficiency ? efficiency : 0).toFixed(2)}%`
                  : "ERR"
                : "No Stat"}
            </PercentBadge>
          </InputGroup.Text>
      ...
      <div className="p-1">
        {!error ? (
          ...
          <span>
            <Badge variant="danger">ERR</Badge> {error}
          </span>
        )}
      </div>
    </Card>
